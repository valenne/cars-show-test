    ...
    <div className='search-manufacturer'>
      <Combobox value={manufacturer}>
        <div className='relative w-full'>
          ...
          <Transition as={Fragment} leave='transition ease-in duration-100' leaveFrom='opacity-0' afterLeave={() => setQuery('')}>
            <Combobox.Options>
              {filteredManufacturer.map((item) =>
                <Combobox.Option key={item} className={({active}) => `relative search-search-manufacturer__option ${active ? 'bg-primary-blue text-white' : 'text-gray-900'}`} value={item}>
                  {item}
                </Combobox.Option>)
              }
            </Combobox.Options>
